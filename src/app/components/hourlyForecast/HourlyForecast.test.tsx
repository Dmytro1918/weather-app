...
interface RechartsMockProps extends React.PropsWithChildren<any> {
    [key: string]: any;
}
...
jest.mock('recharts', () => {
    return {
        ...
        Line: (props: LineMockProps) => {
            const { dataKey } = props;
            return <path data-testid={`line-${dataKey}`} />;
        },
...
const mockHourlyData = [
    {
        dt: 1678881600,
        main: { temp: 293.15, feels_like: 291.15, humidity: 60, pressure: 1012 },
        wind: { speed: 5.5 },
        weather: [{ icon: '01d', description: 'clear sky' }],
    },
    {
        dt: 1678892400,
        main: { temp: 295.15, feels_like: 294.15, humidity: 55, pressure: 1010 },
        wind: { speed: 6.0 },
...
describe('HourlyForecast', () => {
    afterAll(() => {
        try {
            (jest.spyOn(global as any, 'Date') as jest.SpyInstance).mockRestore();
        } catch {
        }
    });
